...
| M2-06 | **toolsearch 延迟工具箱**：用框架 `plugin/toolsearch`：将 M2-02 接入的 MCP server 工具 + 内置大量工具注册为命名空间工具箱；暴露 `tool_search` + `call_tool` 双工具给 Orchestrator；按需（名称/关键字）装载，避免全量注入 context 膨胀（用户 2026-08-03 确认：命名空间/关键字，非语义嵌入）。MCP 动态工具箱命名 `mcp__<server>__<tool>` | ○ | 接入一个 MCP server（mock stdio）→默认不装载其全部工具→模型 tool_search 找到→call_tool 执行→结果返回；context token 不随 MCP 工具数线性膨胀 | M2-02 |
