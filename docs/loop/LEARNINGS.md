...
### 2026-08-20 | 架构 | 多节点 taskrun：外部队列 + lease（M8-03，重要）
- **为什么需要**：框架 inprocess.Service 的 run 记录与执行 goroutine 都在单进程内；多副本部署各副本互不可见（无法分担）、节点崩溃任务即丢。M8-03 把 run 记录外置到共享 SQLite 表 `taskrun_queue`，实现「原子领取 + lease 租约」的多节点执行。
- **原子领取（正确性根基，已踩坑修正）**：ClaimNext 必须在**事务内**「SELECT 待领 id（created_at 升序 FIFO）→ `UPDATE ... WHERE id=? AND status='queued'`（RowsAffected==0 说明被并发节点领取，本轮放弃）→ 按精确 id 读回」。**不要**用「UPDATE 子查询同表选行」或「按 worker 取最新 running 行读回」——前者并发下可能读到已变行（子查询求值时机与写锁获取顺序的耦合不可控），后者在多 running 行下因时间戳存储精度歧义返回错行（实测：20 任务只领到 10 个 + 重复领取，任务「丢失」实为被 UPDATE 成 running 却读回错行、永远不会再被领取）。**SQLite DSN 必须 `_txlock=immediate`**：BeginTx 立即拿写锁（RESERVED），SELECT→UPDATE 全程最新数据、无 `SQLITE_BUSY_SNAPSHOT`「先读快照后写」冲突；busy_timeout(10000) 兜底多进程并发写。
- **status 列是状态机权威**：轻量转换（ClaimNext / UpdateStatusIf nil 分支 / RequeueExpired）只更新 status 列，run_json 内嵌的 Status 可能滞后——scanRow 读回时**以 status 列为准**覆盖 run_json.Status，否则会出现「UPDATE 成功但读回还是旧状态」的假象。
- **lease 与崩溃重拾语义**：领取即绑定 `worker_id + lease_expires_at`；执行中每 Lease/2 续约一次（续约失败→取消本节点执行，防止「本节点继续跑 + 其他节点已重拾」双跑）；RequeueExpired 三分流：canceling（无有效 lease）→canceled 不重试、running+lease 过期+attempts<max→queued（attempts+1 重拾）、超限→failed。**canceling 任务绝不能重试**（否则「取消后又被执行」）。
- **终态乐观写防覆盖**：执行结束先 `UpdateStatusIf(expected=running → terminal)`，失败（状态已变）再试 `canceling → canceled`，仍失败则放弃（可能已被重拾/处理）——避免双节点互相覆盖终态。Cancel 语义：queued→直接 canceled（保证不被执行）；running→canceling + 本节点立即 cancel ctx（其他节点由 cancelWatcher 轮询检测后取消）；执行 goroutine 经 ctx.Canceled 收敛为 canceled。
- **跨节点执行上下文重建**：`SpawnRequest.RunContext`/`RunOptions`/`RuntimeState` 是框架声明的**本地闭包（非跨节点契约）**，queue 模式不依赖它们——ChildSessionID/RequestID 在 Spawn 时确定性生成并持久化（格式与 inprocess 一致 `taskrun:<runID>:<nano>`，transcript 回查兼容）；Timeout 存 `Run.Metadata["taskrun.queue.timeout_ns"]`（Run 结构无 Timeout 字段）；worker 身份从 Run.OwnerUserID/ParentSessionID 重建（WithWorkerUserID/ParentSessionID 注入 ctx，与 WithWorkerIdentity 同款约定，故包装器兼容）。
...
### 2026-08-20 | 架构 | 连接器市场的两个「既有缺陷」修复（M8-08，重要）
- **GORM 零值 bool + default 标签陷阱**：`Enabled bool gorm:"not null;default:true"` 在 `db.Create` 时若字段为零值（false），GORM **省略该列**让 DB 填默认值，并把默认值**回填结构体**——`enabled:false` 会被悄悄置回 true（单测+集成+runtime 三处实测）。修复在 repo 层统一兜底：`CreateMCPServer` 按 **Create 前的期望值**（`wantEnabled := m.Enabled`）在 Create 后显式 `Update("enabled", false)` 并回写 `m.Enabled=false`（不能按 Create 后的 m.Enabled 判断——它已被回填 true，判断恒不触发）。同时 `CreateMCPServerHandler` 把「不传 enabled」的缺省语义显式化为 `true`（否则缺省 false 会被 repo 校正误伤为不启用）。**凡带 default 标签的 bool/标量字段都要警惕 Create 回填**。
- **mcp_servers 唯一索引是单列 name（M2-02 遗留）**：模型只在 `Name` 声明 `uniqueIndex:idx_user_mcp,priority:1`，`UserID` 仅 `index`——idx_user_mcp 实际是**单列 name 全局唯一**，不同用户无法建同名 MCP（连接器市场致命缺陷：每用户都要能导入同名模板 github）。修复：`UserID` 补 `uniqueIndex:idx_user_mcp,priority:1` + `Name` 改 `priority:2`（对齐 Session 的 (user_id, xxx) 复合惯例，user_id 在前利于按用户过滤）；新增迁移 `0013_fix_mcp_servers_composite_unique`：查 sqlite_master 的索引 DDL，单列（不含 user_id）则 `DROP INDEX` + `CREATE UNIQUE INDEX (user_id, name)`（AutoMigrate 只认索引名、不比较列定义，同名索引存在即跳过，必须手动重建）。旧库 name 全局唯一 ⇒ 无跨用户同名数据，重建无冲突，安全幂等。
- **连接器市场设计（可复用）**：模板 = 纯数据 + 纯函数（`internal/marketplace`），`{{KEY}}` 占位符（敏感/个性化字段），导入时按 **env ∪ headers 合并查找表**替换任意位置的占位符（GitHub 的 token 在 headers 里，但用户表单统一收 env 也能替换）；**匹配到占位符的键只作查找源、不冗余落库**（`placeholderKeys()` 预计算模板占位符集合，用户 env 键 ∈ used 则跳过合并）——GitHub 导入后 env 保持为空、token 只出现在 headers 密文。未提供的占位符保留原样（可导入后编辑补齐，Validate 不受影响）。路由注意：**静态段 `/mcp/templates` 必须先于 `/mcp/:id` 注册**，否则被参数路由吞掉。
- **沙箱 /tmp 再次不可用（重申）**：`/tmp` 跨命令视图隔离不可靠（`cp /tmp/x` 报 No such file）——临时二进制/脚本一律放仓库内路径（如 server/mktest_tmp.exe，*.exe 被 gitignore 不提交），用完即删。

### 2026-08-20 | 架构 | 多租户隔离的可复用设计（M8-09，重要）
- **租户聚合 = SQL 子查询实现「结构性隔离」**：`SumUsageRecords` 的 tenant 过滤用 `user_id IN (SELECT id FROM users WHERE tenant_id = ?)`——租户 A 的用量只可能来自 A 的用户，**从查询构造上就不可能包含 B 的用量**（比「程序里按租户取用户再拼 IN」更原子、无竞态）。workspace 维度同理：`usage_records.workspace_key` 冗余列 + `WHERE workspace_key=?`，会话绑定 workspace 时落库（默认目录会话留空不参与聚合）。**预算聚合统计口径与 M3-03 用量记录同源（usage_records）**，新增作用域只是加过滤条件，不新造计数。
- **`EvaluateBudgets` 内部查用户表要容忍「表不存在」**：tenant 分支用 `GetUserByID`（内部 `Preload("Role")`）取 TenantID——在只迁移了 budget/usage 表的旧测试库里 users 表不存在，Preload 报错。处理：`if u, uerr := GetUserByID(...); uerr == nil && u.TenantID != nil`——错误即跳过 tenant 检查（无租户归属），不阻断评估。新测试库需含 Role 表（Preload 依赖）。
- **GORM 指针字段置 NULL 的坑**：`TenantID *uint` 置空（移出租户）不能用「结构体 Update 赋 0」——0 是零值会被省略（且 0 与 nil 语义不同：0 会变 FK 指向不存在租户）。必须 `db.Model(&User{}).Where("id=?").Select("tenant_id").Update("tenant_id", nil)` 显式置 NULL；置非零值用普通 `Update("tenant_id", tid)`。区分「nil=不修改（PUT 缺省）」与「0=移出」在 API 层用 `*uint` 指针判定。
- **Gateway.prepareRun 顺序依赖（M8-09）**：workspace 作用域预算需要「会话绑定的 workspace key」，而绑定发生在 `resolveWorkspaceLocalDir`（持久化 sessions.workspace_id）。故 prepareRun 顺序从「预算→模型→会话→workspace」调整为「会话→workspace 解析→预算→模型」——**凡预算/用量维度依赖运行时解析的归属（workspace/tenant），解析必须先于预算检查**；写 user 消息不耗 token，可提前。
- **磁盘配额 = 目录边界独立统计 + 写入前检查**：`enforceQuota(workdir, quota, delta)` 用 `filepath.Walk` 递归统计 workdir 总大小（跳过单个文件读取失败不阻断），`size+delta > quota` 拒写（返 `ErrDiskQuotaExceeded` 哨兵，Agent 可感知）。file_edit 的 delta = `(len(new)-len(old))*count`（净增量，体积变小放行）；quota<=0 直接放行（不 Walk，零开销）。**配额按 workdir 目录边界独立**——一个 workspace 写爆不影响同用户其他 workspace（隔离本质）。team 模式 Coder 在 worktree 执行，M8-09 按 0 处理（产物 merge 回主 workspace 由单代理路径配额兜底），若需 worktree 级配额可在派生点注入父 workspace 配额。
- **新作用域扩展现有枚举要同步四处**：`model/budget.go` 常量 + `Validate`、`repo.EvaluateBudgets` 评估分支、`api/tenant.ts` 前端类型（BudgetScope 联合类型）、`budgetRequest`（无需改——scope 是自由字符串，Validate 兜底）。漏一处会导致「策略能建但永不生效」或「前端类型不识别」。
- **前端 admin 菜单隐藏用 Set 而非单 key 比较**：`menuOptions.filter((o) => !adminKeys.has(String(o.key)))`——MenuOption.key 是 `Key | undefined`，`Set<string>.has` 需先判空再 String 化（vue-tsc 报 TS2345 的直接原因）。新增 admin 专属页时把 key 加进 `adminKeys` 即可。
