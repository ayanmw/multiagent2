...
### 2026-08-11 11:55 | M4-05 | ✅
- 完成内容：**跨天恢复（M4 自主化第五任务，依赖 M1-16 + M2-04，对齐 PLAN 验收「跑长 Loop 中途 kill 后端 → 重启 → 恢复任务自动续跑且接续已有进展（不重头）」）**。核心痛点：目标契约（M1-11）收敛状态原本只活在内存储存（GoalEnforcer store），进程重启即丢，无法判断哪些 Goal Session 还没跑完；而 artifact（PLAN/PROGRESS/LEARNINGS，M1-16）已落盘、StateEnforcer 会自动回灌。故引入 `automation_runs` 运行记录表作为「跨重启恢复唯一真相源」。① 新增 `model/automationrun.go`：`AutomationRun`（AutomationID/UserID/SessionKey/Channel/Status/Error/Attempts），状态常量 `running/done/failed`，渠道常量 `cron/webhook/recover`。② 新增 `repo/automationrun.go`：`CreateAutomationRun`/`ListUnfinishedAutomationRuns`(筛 status=running)/`ListAutomationRuns`(owner 隔离倒序)/`MarkAutomationRunStatus`(幂等 Updates)。③ 新增 `api/recovery.go`：`RecoverUnfinishedRuns`（进程启动后扫描 running 行，经 `GetAutomationByID` 取回 GoalPrompt 重组 `Request{Channel:ChannelRecover, TeamOverride:&goalTeam, Message:recoveryMessage}` 经统一 Gateway 续跑；automation 缺失→failed；成功→done；失败累加 attempts，达 `maxAttempts`→failed、未达→保留 running 待下次重启续跑）+ `DefaultRecoveryMaxAttempts=3` + `recoveryMessage`（明确「先读 read_state 接续已有进展，不要从头开始」）。④ 接线：`gateway.go` 增 `ChannelRecover` 常量；`scheduler.go`(`runAutomation`) 建 running 记录→成功 done/失败 failed（建表失败仅告警不阻断）；`webhook.go`(`runLoop`) 同款；`main.go` 在 `schedulerSvc.Start` 后 `go api.RecoverUnfinishedRuns(...)` 后台扫描（只针对旧中断会话，与调度器新会话不冲突）；`config.go` 增 `RECOVERY_MAX_ATTEMPTS`(默认 3) + `RecoveryMaxAttempts()`（≤0 回落 3），并补本地 `DefaultRecoveryMaxAttempts` 常量（避免反向依赖 api 包）。⑤ `repo/migrate.go` 增 `0005_add_automation_runs` 迁移 + `baselineModels()` 登记 `AutomationRun`（取代纯 AutoMigrate）。
- 验证：`CGO_ENABLED=0 go build ./...` ✓ | `go vet ./...` ✓。新增测试 **11 例全 PASS**（纯 Go `glebarez/sqlite`，免 gcc，无密钥/本地路径泄漏）：`repo/automationrun_test.go` 2 例（CRUD：running/done/failed 过滤 + MarkAutomationRunStatus 幂等回写；按 user 隔离倒序）；`api/recovery_test.go` 7 例（无未收敛→0、成功→done 且请求含恢复提示+ChannelRecover+目标契约覆盖+沿用原 session、done 不续跑、automation 缺失→failed、失败未达上限→保留 running 且 attempts 累加、失败达上限→failed）；`scheduler` 补 2 例（成功→done、失败→failed，按 automation 主键过滤规避 cache=shared 内存库跨测试共享）。`gofmt` 干净。已知环境限制：`repo/model/session/role` 等历史 `go-sqlite3`(CGO) 用例在 `CGO_ENABLED=0` 无 gcc 下编译失败（既有，非本任务缺陷；本任务测试全部改用纯 Go glebarez/sqlite）。`config.go` 此前引用未定义 `DefaultRecoveryMaxAttempts` 的编译错误已修复（本地补常量）。
- 下一步：PLAN 中 **M4-06（无人值守 Loop 运行模式：Mode=Unattended，SafeExecutor deny 默认 + 预算护栏 + 检查点排队，依赖 M3-04/M3-05/M4-04）** 成为下一个 ○。

---

### 2026-08-11 12:57 | M4-06 | ✅
- 完成内容：**无人值守 Loop 运行模式（M4 自主化第六任务，依赖 M3-04 预算护栏 + M3-05 人工检查点 + M4-04 Channel 层，对齐 PLAN 验收「多步 Goal 在无人值守下自动跑完并产出 PR/报告；中途危险操作进检查点队列待批」）**。核心是把「执行器运行模式」从隐式默认 upgrade 为显式可配置 + 自动 Loop 强制 Unattended，并把此前只挂在 Web Chat/SSE 入口的预算护栏下沉到统一 Gateway，让自主 Loop 路径也受保护。① **配置层** `internal/config/config.go`：新增 `RunModeUnattended="unattended"`/`RunModeAttended="attended"` 常量 + `runMode` 字段；`Load()` 解析 `RUN_MODE`（缺省回落 `unattended`）；访问器 `RunModeString()`（原始串）/ `IsUnattended()` / `ExecutorMode()`（nil 安全，映射 `executor.ModeUnattended`/`ModeInteractive`）。② **执行器模式贯通**：`internal/tool/codeact.go` 增 `normalizeExecutorMode`（非法回落 Unattended）+ `NewCodeAct(...,mode)`；`git.go` 的 `NewGitExecutor/NewGitTools/NewCodeActWithGit` 同签转发；`agent/factory.go` 的 `Deps.ExecutorMode` 经 `NewCoder` 注入 codectool；`engine/engine.go` 的 `ModelConfig.ExecutorMode` 注入 team 分支；`api/codeact.go`/`workspace.go`/`worktree/worktree.go`/`taskrun/taskrun.go` 生产调用点补 `executor.ModeUnattended`；`main.go` 的 `buildGateway` 透传 `cfg.ExecutorMode()`、workerFactory 用 `ModeUnattended`。③ **Gateway 强制 Unattended + 预算集中拦截** `api/gateway.go`：`GatewayConfig.ExecutorMode` + `resolveExecutorMode(ch)`（cron/webhook/recover 渠道无视配置强制 Unattended，Web/CLI/IM 跟随配置）；`prepareRun` 顶部统一调 `repo.EvaluateBudgets` 做预算护栏（此前仅 Web 入口有，自主 Loop 无）——超限写 `budget:enforce` 审计并返回 `&BudgetExhaustedError{Eval: budgetEv}`；`exMode` 经 resolver 计算后注入 `NewCodeActWithGit` 与 `engine.New(ExecutorMode: exMode)`。④ `chat.go`/`sse.go` 撤除入口处重复预算块，改为 post-run `errors.As(&be)` 转 429 / SSE RUN_ERROR 事件；新增哨兵 `ErrBudgetExhausted` + 类型化 `BudgetExhaustedError`（`Unwrap`→`ErrBudgetExhausted`，携带 `BudgetEvaluation`）。
- 验证：`CGO_ENABLED=0 go build ./...` ✓ | `go vet ./...` ✓。新增测试 **8 例全 PASS**：`config_test.go` 3 例（`TestRunModeDefaults` 零值/空→Unattended、`TestRunModeAttended`→Interactive、`TestRunModeInvalidFallsBack` 非法→Unattended）；`gateway_test.go` 5 例（`TestGateway_ResolveExecutorMode_AutonomousForcesUnattended` 自主渠道强制 Unattended 即便配置=attended、Web/CLI/nil 跟随配置、`TestGateway_ResolveExecutorMode_DefaultUnattended` 默认全 Unattended、`TestBudgetExhaustedError` errors.As/Is(ErrBudgetExhausted)/Eval 透传）。`gofmt` 干净；同步更新 11 处测试调用点补 `executor.ModeUnattended` 形参（tool/readonly/git/codeact/engine/engine_git/worktree/taskrun/workspace/api 等）。已知环境限制：`internal/repo` 中历史 `go-sqlite3` CGO 用例在 `CGO_ENABLED=0` 无 gcc 下失败（既有，非本任务缺陷）。diff 扫描：无 `anmingwei`、本地绝对路径、硬编码密钥。
- 下一步：PLAN 中 **M4-07（通知/结果回发 outbound：站内信表 notifications + Webhook 回调占位 + 邮件占位 + 前端通知中心，依赖 M4-02）** 成为下一个 ○。

### 2026-08-11 15:40 | M4-07 | ✅
- 完成内容：**通知/结果回发 outbound（M4 自主化第七任务，依赖 M4-02 Cron 调度器，对齐 PLAN 验收「Loop 完成 → 通知中心出现一条；webhook 目标收到回调（可用 mock）」）**。把「自主 Loop 的运行结果」从只写日志升级为「站内信落库 + 可选出站 Webhook 回调」双通道，并配前端通知中心与顶栏红点。① **领域/持久化** `model/notification.go`（Notification：user_id 归属 + type[success/failure/checkpoint] + title/message + ref_kind/ref_id + read_at 软未读）+ `repo/notification.go`（Create/List 分页 owner 隔离/CountUnread/MarkRead/MarkAllRead）。② **出站服务** `internal/notify/notify.go`：`Notifier` 接口 + `Service`（best-effort 写库，绝不阻塞自主 Loop；设了 `WEBHOOK_NOTIFY_URL` 则 POST 回调，httptest 可验证）+ 构造器 `NewSuccess/NewFailure/NewCheckpoint` + `shortReply` 截断 200 字。③ **三入口 + 检查点全部挂通知**：`scheduler.go`（成功→NewSuccess / 失败→NewFailure，nil 安全）、`webhook.go`（成功→NewSuccess，`.WithNotifier` 链式保测试兼容）、`recovery.go`（包级 `SetRecoveryNotifier`，跨重启恢复完成→NewSuccess）、`gateway.go`（checkpointer 闭包在生成检查点后→NewCheckpoint，覆盖 cron/webhook/recover 三渠道）。④ **迁移/权限/路由/配置**：`migrate.go` baseline 注册 + 迁移 `0006 add_notifications`；`role.go` developer 补 `notifications:read/write`、viewer 补 `notifications:read`（幂等 seed）；`main.go` 注册 `GET /notifications`、`POST /notifications/:id/read`、`POST /notifications/read-all`（RBAC notifications:read|write）+ 构造 notifier 注入四处；`config.go` 增 `WEBHOOK_NOTIFY_URL` env + 访问器。⑤ **前端**：`web/src/api/notification.ts`（list/markRead/markAllRead + 类型映射）+ `NotificationView.vue`（列表 + 未读优先排序 + 类型筛选 + 单条/全部已读）+ 路由 `notifications` + 菜单项 + 顶栏铃铛 `NBadge` 红点（30s 轮询未读）。
- 验证：`go build ./...` ✓ | `go vet ./...` ✓。通知相关测试 **全 PASS（纯 Go glebarez sqlite，免 gcc）**：`notify/notify_test.go` 2 例（写库 / 回调触发）+ `scheduler/scheduler_test.go::TestScheduler_NotifiesOnSuccess` 1 例 + `api/webhook_test.go::TestWebhookHandler_NotifiesOnSuccess` 1 例 + `cmd/server/notification_test.go::TestNotifications_ListAndRead` 1 例（developer 可见/未读计数/标记已读/view 403 全流程）。**修复本轮真实 bug**：`TestScheduler_NotifiesOnSuccess` 的 automation 名误用既有测试已占用的 `every-min`，与 `cache=shared` 内存库跨测试共享触发 UNIQUE 约束失败 → 改名为 `notify-success` 解决。前端 `vue-tsc --noEmit` ✓ | `vite build` ✓（NotificationView chunk 4.48 kB）。已知环境限制：`internal/repo` 9 例历史 `go-sqlite3` CGO 用例在 `CGO_ENABLED=0` 无 gcc 下失败（既有，非本任务缺陷，与改动无关）。diff 扫描：无 `anmingwei`、本地绝对路径、硬编码密钥。
- commit 8c4dcab 已推 origin/main。下一步：PLAN 中 **M4-08（自动化管理前端：AutomationView 列表/创建/启用停用/运行历史，依赖 M4-01 + M4-07）** 成为下一个 ○。

### 2026-08-11 17:05 | M4-08 | ✅
- 完成内容：**自动化管理前端（M4 自主化第八任务，依赖 M4-01 Automation 数据模型/CRUD + M4-07 通知中心；对齐 PLAN 验收「前端可建 cron Automation 并看到下次运行时间；运行历史可查」）**。把「自动化」从纯后端模型/API 升级为可运营的前端管理面，并复用 M3-05 人工检查点审批。① **后端运行历史端点** `internal/api/automation.go`：新增 `GET /api/automations/:id/runs`（owner-scoped，需 `automations:read`）→ `ListAutomationRunsHandler`，复用 `repo.ListAutomationRuns`（M4-05）按 user_id 拉全量后按 `automation_id` 过滤、按 created_at 倒序；`automationRunView`（id/automation_id/session_key/channel/status/error/attempts/时间戳）；`main.go` 注册路由（M4-01 已有 CRUD 五路由）。② **前端 API** `web/src/api/automation.ts`：封装 `listAutomations/createAutomation/updateAutomation/deleteAutomation/listAutomationRuns` + 触发类型(`cron`/`webhook`)/渠道(`cron`/`webhook`/`recover`)/状态(`running`/`done`/`failed`) 中文标签映射。③ **前端视图** `web/src/views/AutomationView.vue`：列表表格（名称/触发器 Tag/表达式(等宽)/Goal Prompt/启用 NSwitch/上次·下次运行时间/操作列含编辑·运行历史·删除 NPopconfirm）；创建·编辑抽屉（名称/cron 表达式 或 webhook 事件说明/Goal Prompt/启用）；启用停用经 NSwitch 直发 PUT；运行历史抽屉按 automation_id 拉取并着色渠道/状态；顶部「待审批检查点」卡片复用 `checkpoint` API + M3-05 审批（approve/reject）。④ **路由与菜单**：`router/index.ts` 注册 `automations` 路由；`DefaultLayout.vue` 菜单新增「自动化」项（svg 图标）。
- 验证：本轮核验 `CGO_ENABLED=0 go build ./...` ✓ | `go vet ./...` ✓；前端 `vue-tsc --noEmit` ✓ | `vite build` ✓（AutomationView chunk 9.78 kB，gzip 3.97 kB）。说明：本任务实现由先前一轮提交（commit 4823ab9 「feat(M4-08): 自动化管理前端」）完成，但其 PLAN/PROGRESS/memory bookkeeping 被遗漏未写；本轮重新核验编译与类型检查全绿，并补齐三项书签记录，闭环 M4-08。diff 扫描（4823ab9）：无 `anmingwei`、本地绝对路径、硬编码密钥。
- commit 4823ab9 已推 origin/main（实现）。下一步：PLAN 中 **M4-09（集成验证 E2E 自主化：建定时 Automation → 到点自动跑 Loop → 产出 PR/报告 → 跨重启恢复 → 完成通知 全链路；补 cron/webhook/恢复单测，依赖 M4-01..08）** 成为下一个 ○。

### 2026-08-11 18:13 | M4-09 | ✅
- 完成内容：**集成验证 E2E（自主化收口，M4 第九任务，依赖 M4-01..08 全部 ✅）**。把「定时触发 → 自动 Loop → 跨重启恢复 → 完成通知」四条主链路在 HTTP 层用真实编排串起来，补齐 cron/webhook/恢复三类单测。新增 `server/cmd/server/m4_integration_test.go::TestM4_Autonomy_E2E`，进程内复用 `buildRouter`/`buildGateway`/`e2eClient`/`newM1HTTPMockLLM`/`parseAGUI`（不调真实 LLM），覆盖：① **Cron（M4-02/07）**：建 cron Automation 并把 `NextRun` 拨到过去 → `scheduler.TickSync` 触发 → 断言 run 落库 done + 成功通知写入 + session 创建 + `LastRun` 更新；② **Webhook（M4-03/07）**：建 webhook Automation → 读 `webhook_token` → `POST /api/webhooks/:token` 返回 202 且 body 带 `automation_id` → 等待异步 run done + 成功通知；③ **恢复（M4-05）**：直接插 `model.AutomationRun(status=running)` → 调 `api.RecoverUnfinishedRuns` 走真实 Gateway 重跑 → 断言 run 转 done、session 含 ≥2 条消息（user+assistant）、成功通知写入。聚焦 M4 编排串联（团队/Goal 契约由 M1 单测与引擎测试已覆盖），故 E2E 用单代理 `TeamConfig{}` 规避 M1-11「goal 契约需非流式 mock」陷阱（测试注释已写明）。辅助函数：`waitRunDone`/`assertRunHasSession`/`assertNotification`。
- 验证：`CGO_ENABLED=0 go build ./...` ✓ | `go vet ./...` ✓（修复一处未用 import `net/http/httptest`）| `go test ./cmd/server/ -run TestM4_Autonomy_E2E -v -count=1` → `--- PASS: TestM4_Autonomy_E2E (0.61s)` / `ok ... 0.685s`；日志确认三条链路均生效：cron 触发→run done+通知；webhook 202+异步 done+通知；recovery→Gateway 重跑 Loop、run done、session 持久化 2 消息、成功通知落库。已知环境限制：`internal/repo` 历史 `go-sqlite3` CGO 用例在 `CGO_ENABLED=0` 无 gcc 下失败（既有，非本任务缺陷）。
- commit（待 push）：post-commit hook 自动推 origin/main。下一步：M4 全部 ✅，**M5（进化：CLI+Knowledge RAG+evolution 技能飞轮+evaluation 回归+promptiter+A2A）门槛「M4 全部 ✅」已满足**，但 PLAN 中 M5-01..09 均为 ○ 且未在本轮拾取（按 LOOP 协议每轮仅一个 ○；下一轮可起始 M5-01）。

### 2026-08-11 19:38 | M5-01 | ✅ CLI 骨架（cobra + bubbletea，复用 REST+SSE API）
- 新建独立 Go 模块 `tool/cli`（module `github.com/ayanmw/multiagent2/tool/cli`），遵循「工具子项目放 ./tool/<name>/」约定，与 server 模块解耦。
- 复用后端同一套 API 契约（对齐 web/src/api/* 与 server/internal/api/*）：`internal/api/client.go` 实现 Login/Register/Me/CreateSession/ListSessions/GetSession/ListTaskRuns + `StreamChat`（手动解析 AG-UI SSE 帧 `data: {...}\n\n`，与前端 streamChat 等价）；`internal/config` 管理本地配置（<UserConfigDir>/gm-agent-cli/config.json，支持 GM_AGENT_BASE_URL / GM_AGENT_TOKEN 环境变量与 --base-url/--token 覆盖）。
- 命令（cobra）：`login`/`logout`/`me`/`sessions`/`chat`/`tasks`。`chat` 一次性模式流式打印助手文本（stdout 仅文本、工具调用/错误走 stderr 便于管道）；`--repl` 进入 bubbletea 交互对话（TTY 守卫，流式经 goroutine+channel 推入 bubbletea Cmd 循环）。
...
### 2026-08-12 14:30 | M5-04 | ✅ evolution 前端 + 审批发布（依赖 M5-03 + M2-03）
...
### 2026-08-13 17:23 | M5-07 | ✅ A2A 对外协议服务端落地（Agent Card + JSON-RPC 任务入口 + 集成测试）
- A2A（Agent-to-Agent，Google 规范 0.2.0）服务端最小可用端点（M5 进化第七任务，依赖 M4）。此前一轮已铺底 `internal/a2a`（协议类型包，无框架依赖：AgentCard/Task/JSONRPC 信封/错误码）+ `internal/api/a2a.go`（AgentCardHandler 公开 + A2AHandler 经 Gateway.Run）+ `main.go` 的 `/.well-known/agent.json` 路由，但 handler 漏 `repo` import 导致编译失败、且 `/api/a2a` JSON-RPC 路由未注册。本轮收口：
  ① `internal/api/a2a.go` 补 `repo` import（用 `repo.NewSessionKey()` 生成 task/session id），其余 `currentUserID`/`ChannelA2A`/`Gateway.Run`/`BudgetExhaustedError` 均已就绪；
  ② `cmd/server/main.go` 在受保护组内注册 `POST /api/a2a`（沿用 AuthMiddleware，外部 A2A client 用 X-API-Key/Bearer 鉴权，与 Agent Card 声明 apiKey 一致）；
  ③ `A2AHandler` 校验 JSON-RPC 2.0 信封（jsonrpc 字段/方法 message|tasks/send/缺文本）→ 返回规范错误码（-32600/-32601/-32602），任务 id 复用为会话 key 实现多轮续聊，经统一 Gateway 跑引擎（复用会话串行锁、多轮记忆、预算护栏、用量计量），外部任务以 `ChannelA2A` 标记，与 Web/CLI/定时/Webhook 收敛到同一 Runner；预算耗尽映射 429（-32003）、模型失败映射 502（-32002）；返回 A2A `Task`（id/status=completed/history/artifacts）作为 JSON-RPC result。
...
### 2026-08-20 08:15 | M7.5-03 | ✅ 并发与压测——四场景压测套件全绿 + 暴露并修复 worktree 并发 merge 竞态
- **交付物**：
  - 新增 `internal/smoke/load_test.go`（M7.5-03 压测套件，4 场景）：① `TestLoad_ConcurrentChat` 多用户并发对话（N 用户×3 轮带历史回灌，统计 P50/P90/P99/max，断言 Mock 请求数严格等于调用数=无连接泄漏）；② `TestLoad_SSELongConnection` SSE 长连接稳定性（每流 50 分片、慢速消费 2ms/事件、服务端活跃连接计数，断言文本完整+结束后活跃连接归零）；③ `TestLoad_TaskrunFanout5Plus` taskrun 扇出 ≥5 子任务（Orchestrator create_goal→start_task_run(async)×N→list_task_runs 轮询收敛→update_goal complete；N 个 worker 并发在独立 worktree 写 task-k.txt→commit→Observer 自动 merge 回主分支；断言主分支全文件+git log 含 N 提交+worktree 清理+goal complete+控制器 N run 全 completed）；④ `TestLoad_SQLiteWriteContention` SQLite 写锁（文件型库对齐生产 SetMaxOpenConns(1)，写者各自会话+8 写者共享会话，断言 0 lock 错误+行数精确+墙钟上限）。规模环境变量可调（LOAD_N_USERS/LOAD_SSE_CONNS/LOAD_SSE_CHUNKS/LOAD_TASKRUN_FANOUT/LOAD_SQLITE_WRITERS），`LOAD_REPORT` 落盘报告、`LOAD_FANOUT_DEBUG` 打印 mock 决策序列。
  - **压测报告** `docs/loadtest-report.md`：放大规模指标（50 用户并发 P99=68ms / 25 SSE 长连接 P99=202ms 活跃连接归零 / 10 子任务扇出 1.6s 全 merge / 32+8 SQLite 写者 1000 条 0 lock 错误）+ 缺陷修复说明 + 复现方式 + 已知边界。
  - **修复并发缺陷（压测暴露）**：`worktree.Manager.Finalize` 并发 merge 竞态——多子任务同时收敛时 Observer 并发触发 Finalize，多个 `git merge --no-ff` 同时写同一主仓库 index/HEAD 互相踩踏（多数 exit_code=128），子任务产物丢失（git log --all 只剩 1 个 worker commit、其余分支被 worktree 检出悬置）。修复：`Manager` 新增 `mergeMu sync.Mutex` 串行化 merge 段（git 单写者模型）+ merge 失败路径补 `log.Printf` 错误详情（含 branch/err/out）。回归保护：worktree 包新增 `TestManager_ConcurrentFinalize`（6 worktree 并发提交+并发 Finalize，断言全部 merge 成功、产物完整、分支与目录清理）。
...
- Commit 6c5b104 已推 origin/main。下一步：首个 ○ = M8-04（Knowledge RAG 升级 PG/pgvector，依赖 M5-02）；M7.5-04 待用户提供集群后恢复。

### 2026-08-20 12:55 | M8-04 | ✅ Knowledge RAG 升级 PG/pgvector（万级检索 + 并发无退化）
- 交付：① `internal/knowledge/store/pgvector.go`——pgx v5 stdlib（纯 Go 无 CGO）实现框架 `vectorstore.VectorStore` 全接口 + `ListDocuments/DeleteDocument`：`PGPool` 共享连接池（`ForKB(kbID)` 轻量视图，避免每 KB 开池）；幂等建表 `kb_vectors`（id/kb_id/doc_name/name/content/embedding vector(N)/metadata jsonb）；启动自动探测 pgvector 扩展（缺失报错带部署指引）→ 维度一致性校验（不匹配提示 DROP TABLE 重建）→ HNSW（≥0.5.0）→ IVFFlat → 无索引三级降级；检索 `1-(embedding<=>$v)` 余弦相似度 + metadata `@>` jsonb 过滤 + IDs `ANY($n::text[])` 全下推 SQL；参数全走 database/sql 兼容类型（向量/JSON/数组以字符串字面量 + cast）。② `store.VectorStore` 接口（框架接口 + List/DeleteDocument）+ Manager 工厂化：`NewManager` 默认 SQLite（零改动），`NewManagerWithStoreFactory` 注入 PG；内部 store 调用统一走工厂。③ 装配：config `KB_STORE=sqlite|pgvector`（默认 sqlite 向后兼容）+ `KB_PG_DSN`（pgvector 缺失 fatal，杜绝静默退化）/`KB_PG_DIM`(256)/`KB_PG_POOL`(10)，校验抽纯函数 `validateKnowledgeStore` 可测；api 5 个 handler（Delete/Index/ListDocs/DeleteDoc/Search）签名改收 `*knowledge.Manager`；main.go `buildKnowledgeManager` 按后端分支 + `buildKnowledgeRetriever` 复用。④ 文档 `docs/knowledge-pgvector.md`（部署/切换/索引策略/万级实测/迁移/运维）。
